...
void run_sim(double *u, double *u0, double *u1, double *pebbles, int n, double h, double end_time)
{
  ...
  //tmpptr = (double*)malloc(sizeof(double) * n * n);

  /* put the inital configurations into the calculation arrays */
  //memcpy(uo, u0, sizeof(double) * n * n);
  //memcpy(uc, u1, sizeof(double) * n * n);

  #pragma omp parallel for private(idx,j) num_threads(nthreads) schedule(dynamic)
  for(i=0;i<n;i++){
  for(j=0;j<n;j++){
  idx = j + i*n;
  uo[idx] = u0[idx];
  ...
  #pragma acc data copy(un[:n*n],uc[:n*n],uo[:n*n],pebbles[:n*n])// copyout(un[:n*n],uc[:n*n],uo[:n*n],pebbles[:n*n])
  while(1)
  {
